...
class VideoStreamServer:
    def __init__(self, host="0.0.0.0", port=8080):
        ...
        self.depth_queue = queue.Queue(maxsize=10)  # Buffer para mapas de profundidad
        ...
        self.app.add_url_rule("/depth_map", "depth_map", self.depth_map)
    ...
    def index(self):
        ...
        <body>
            ...
            <a href="/depth_map">Ver mapa de profundidad</a>
        </body>
        </html>
        """
        ...
        try:
            ...
            while True:
                ...
                with self.processing_lock:
                    ...
                    # Genera un mapa de profundidad simple usando gradientes
                    depth_map = cv2.Laplacian(gray_frame, cv2.CV_64F)
                    depth_map = cv2.convertScaleAbs(depth_map)

                    if not self.frame_queue.full():
                        self.frame_queue.put(resized_frame)
                    if not self.depth_queue.full():
                        self.depth_queue.put(depth_map)
    ...
    def generate_depth_map(self):
        while True:
            if not self.depth_queue.empty():
                with self.processing_lock:
                    depth_map = self.depth_queue.get()
                    ret, buffer = cv2.imencode(".jpg", depth_map)
                    if not ret:
                        continue
                    depth_map = buffer.tobytes()
                yield (
                    b"--frame\r\n"
                    b"Content-Type: image/jpeg\r\n\r\n" + depth_map + b"\r\n"
                )
